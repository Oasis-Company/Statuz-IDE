# Statuz IDE Codebase Guide

The Statuz IDE codebase is not as intimidating as it seems!

Most of Statuz IDE's code lives in the folder `src/vs/workbench/contrib/statuz/`.

The purpose of this document is to explain how Statuz IDE's codebase works. If you want build instructions instead, see [Contributing](HOW_TO_CONTRIBUTE.md).

## Statuz IDE Codebase Guide

### VSCode Rundown
Here's a VSCode rundown if you're just getting started with Statuz IDE. You can also see Microsoft's [wiki](https://github.com/microsoft/vscode/wiki/Source-Code-Organization) for some pictures. VSCode is an Electron app. Electron runs two processes: a **main** process (for internals) and a **browser** process (browser means HTML in general, not just "web browser").
<p align="center" >
...
### Statuz IDE's LLM Message Pipeline

Here's a picture of all the dependencies that are relevant between the time you first send a message through Statuz IDE's sidebar, and the time a request is sent to your provider.
...
Statuz IDE has two types of Apply: **Fast Apply** (uses Search/Replace, see below), and **Slow Apply** (rewrites whole file).
...
>>>>>>> UPDATED
```
This is what allows Statuz IDE to quickly apply code even on 1000-line files. It's the same as asking the LLM to press Ctrl+F and enter in a search/replace query.
...
When Statuz IDE wants to change your code, it just writes to a text model. This means all you need to know to write to a file is its URI - you don't have to load it, save it, etc. There are some annoying background URI/model things to think about to get this to work, but we handled them all in `statuzModelService`.

### Settings Inner Workings
We have a service `statuzSettingsService` that stores all your Statuz IDE settings (providers, models, global settings, etc). Imagine this as an implicit dependency for any of the core Statuz IDE services:
...
For additional references, the Statuz IDE team put together this list of links to get up and running with VSCode.
<details>
...
Statuz IDE is no longer an extension, so these links are no longer required, but they might be useful if we ever build an extension again.
