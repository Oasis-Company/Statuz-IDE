...
const ButtonLeftTextRightOption = ({ text, leftButton }: { text: string, leftButton?: React.ReactNode }) => {
	...
	return <div className='flex items-center text-statuz-fg-3 px-3 py-0.5 rounded-sm overflow-hidden gap-2'>
...
const SimpleModelSettingsDialog = ({
	...
	const sourcecodeOverridesLink = `https://github.com/Oasis-Company/Statuz-IDE/blob/main/src/vs/workbench/contrib/statuz/common/modelCapabilities.ts#L146-L172`
	...
	return (
		...
		>
			{/* MODAL */}
			<div
				className="bg-statuz-bg-1 rounded-md p-4 max-w-xl w-full shadow-xl overflow-y-auto max-h-[90vh]"
			...
			>
				<div className="flex justify-between items-center mb-4">
					...
					<button
						onClick={onClose}
						className="text-statuz-fg-3 hover:text-statuz-fg-1"
					>
						<X className="size-5" />
					</button>
				</div>
				...
				{/* Informational link */}
				{overrideEnabled && <div className="text-sm text-statuz-fg-3 mb-4">
				...
				<textarea
					key={overrideEnabled + ''}
					ref={textAreaRef}
					className={`w-full min-h-[200px] p-2 rounded-sm border border-statuz-border-2 bg-statuz-bg-2 resize-none font-mono text-sm ${!overrideEnabled ? 'text-statuz-fg-3' : ''}`}
...
export const ModelDump = ({ filteredProviders }: { filteredProviders?: ProviderName[] }) => {
	...
	return <div className=''>
				...
				<div className="flex items-center gap-2 w-fit">
					...
					{disabled ? null : (
						<div className="w-5 flex items-center justify-center">
							...
							>
								<Plus size={12} className="text-statuz-fg-3 opacity-50" />
							</button>
						</div>
					)}
					...
					<div className={`w-5 flex items-center justify-center`}>
						{type === 'default' || type === 'autodetected' ? null : <button
							...
							className={`${hasOverrides ? '' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}
						>
							<X size={12} className="text-statuz-fg-3 opacity-50" />
						</button>}
					</div>
				</div>
			</div>
		...
		{showCheckmark ? (
				...
				<form className="flex items-center gap-2">
					...
					<ErrorBoundary>
						<VoidCustomDropdownBox
							...
							className="max-w-32 mx-2 w-full resize-none bg-statuz-bg-1 text-statuz-fg-1 placeholder:text-statuz-fg-3 border border-statuz-border-2 focus:border-statuz-border-1 py-1 px-2 rounded"
			...
			<div
				className="text-statuz-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer mt-4"
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-statuz-fg-3 text-sm list-decimal select-text'>
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	return <>
		...
		/>
		<span className="text-statuz-fg-3 text-xs">{desc}</span>
	</>
}
...
const MCPServerComponent = ({ name, server }: { name: string, server: MCPServer }) => {
	...
	return (
		<div className="border border-statuz-border-2 bg-statuz-bg-1 py-3 px-4 rounded-sm my-2">
						...
						${server.status === 'success' ? 'bg-green-500'
							: server.status === 'error' ? 'bg-red-500'
								: server.status === 'loading' ? 'bg-yellow-500'
									: server.status === 'offline' ? 'bg-statuz-fg-3'
										: ''}
					`}></div>

					{/* Server name */}
					<div className="text-sm font-medium text-statuz-fg-1">{name}</div>
				</div>
				...
				<div className="mt-3">
					<div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
						{(server.tools ?? []).length > 0 ? (
							...
							<span className="text-xs text-statuz-fg-3">No tools available</span>
						)}
					</div>
				</div>
			)}
			...
			{isOn && server.command && (
				<div className="mt-3">
					<div className="text-xs text-statuz-fg-3 mb-1">Command:</div>
					<div className="px-2 py-1 bg-statuz-bg-2 text-xs font-mono overflow-x-auto whitespace-nowrap text-statuz-fg-2 rounded-sm">
						{server.command}
					</div>
				</div>
			)}
...
const MCPServersList = () => {
	...
	if (mcpServiceState.error) {
		content = <div className="text-statuz-fg-3 text-sm mt-2">
						...
						<div className='flex flex-col gap-12'>
									...
									<div className='flex flex-col gap-y-8 my-4'>
										<ErrorBoundary>
											...
											<div>
												...
												<div className='text-sm text-statuz-fg-3 mt-1'>
													<span>
														Experimental.{' '}
													</span>
													<span
												...
												<div className='my-2'>
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															...
															/>
															<span className='text-statuz-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.enableAutocomplete ? 'Enabled' : 'Disabled'}</span>
														</div>
													</ErrorBoundary>
													...
													<ErrorBoundary>
														<div className={`my-2 ${!settingsState.globalSettings.enableAutocomplete ? 'hidden' : ''}`}>
															<ModelDropdown featureName={'Autocomplete'} className='text-xs text-statuz-fg-3 bg-statuz-bg-1 border border-statuz-border-1 rounded p-0.5 px-1' />
														</div>
													</ErrorBoundary>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-statuz-fg-3 mt-1'>Settings that control the behavior of the Apply button.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														...
														/>
														<span className='text-statuz-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncApplyToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncApplyToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'Apply'} className='text-xs text-statuz-fg-3 bg-statuz-bg-1 border border-statuz-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
										...
										<div>
											...
											<div className='text-sm text-statuz-fg-3 mt-1'>{`Tools are functions that LLMs can call. Some tools require user approval.`}</div>
											...
											<div className='my-2'>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														...
														/>
														<span className='text-statuz-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.includeToolLintErrors ? 'Fix lint errors' : `Fix lint errors`}</span>
													</div>
												</ErrorBoundary>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														...
														/>
														<span className='text-statuz-fg-3 text-xs pointer-events-none'>Auto-accept LLM changes</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
											...
											<div className='my-2'>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														...
														/>
														<span className='text-statuz-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.showInlineSuggestions ? 'Show suggestions on select' : 'Show suggestions on select'}</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-statuz-fg-3 mt-1'>Settings that control the behavior of the commit message generator.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														...
														/>
														<span className='text-statuz-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncSCMToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncSCMToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'SCM'} className='text-xs text-statuz-fg-3 bg-statuz-bg-1 border border-statuz-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									...
									<h4 className={`text-statuz-fg-3 mb-4`}>{`IDE settings, keyboard settings, and theme customization.`}</h4>
									...
									<div className='my-2'>
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2 my-2'>
												...
												/>
												<span className='text-statuz-fg-3 text-xs pointer-events-none'>{'Opt-out (requires restart)'}</span>
											</div>
										</ErrorBoundary>
									</div>
								</div>
									...
									<div className='my-4'>
										<ErrorBoundary>
											<div className='flex items-center gap-x-2'>
												...
												/>
												<span className='text-statuz-fg-3 text-xs pointer-events-none'>
