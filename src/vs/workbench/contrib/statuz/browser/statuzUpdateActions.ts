...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	...
	if (res?.action) {
		...
		if (res.action === 'reinstall') {
			primary.push({
				...
				run: () => {
					...
					window.open('https://github.com/Oasis-Company/Statuz-IDE/releases')
				}
			})
		}
		...
		primary.push({
			...
			run: () => {
				const { window } = dom.getActiveWindow()
				window.open('https://github.com/Oasis-Company/Statuz-IDE')
			}
		})
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `Statuz IDE Error: There was an error checking for updates. If this persists, please get in touch or reinstall Statuz IDE [here](https://github.com/Oasis-Company/Statuz-IDE/releases)!`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
